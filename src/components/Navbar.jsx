...
const Navbar = () => {
  ...
  return (
    ...
    {menuOpen && (
        <div className="md:hidden  bg-stone-950/30 backdrop-blur-lg
        rounded-xl flex flex-col space-y-4 m-2 min-w-[200px]">
            {LINKS.map((link, index) => (
                <a href={link.href} key={index} className="text-white px-4
                hover:text-stone-400 transition duration-300" onClick=
                {handleLinkClick}>
                    {link.label}
                </a>
            ))}
